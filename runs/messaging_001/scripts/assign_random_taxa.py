...
# def plot_seg(seg, rgb, colors, colormap, num_discrete_values, save, output_png_filename):
...
def main():
    ...
    for seg_name in args.seg_names:
        ...
        for index, row in cell_properties.iterrows():
            ...
            taxon = random.randint(1,args.num_taxa)
        ...
        # seg_taxa = seg.copy()
        # print('map_dict_taxa[39]',map_dict_taxa[39])
        seg_taxa = seg.copy().astype(float)
        seg_taxa[seg_taxa == 0] = np.nan
        for k, v in map_dict_taxa.items():
            seg_taxa[seg==k] = v
            # print('k,v',k,v)
        # print("seg_taxa[0:15,0:15]",seg_taxa[100:150,100:150])
        if args.save == 'T':
        ...
        # taxa_cmap = taxa_cmap(np.linspace(0,1,args.num_taxa))
